...
public class ItemStickyRice extends ItemColored{

	public ItemStickyRice(Block block) {
		super(block,false);
	}
	@SideOnly(Side.CLIENT)
	public int getColorFromItemStack(ItemStack itemStack, int damage) {
		return GtiBlocks.stickyRice.getRenderColor(itemStack.getItemDamage());
	}
		...
		if (movingobjectposition == null) {
			...
			if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
				...
				if (world.getBlock(i, j, k).getMaterial() == Material.clay
					...
					world.setBlock(i, j + 1, k, GtiBlocks.stickyRice);
