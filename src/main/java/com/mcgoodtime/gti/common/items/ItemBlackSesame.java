...
public class ItemBlackSesame extends ItemColored {

	public ItemBlackSesame(Block block) {
		super(block, false);
	}

	@SideOnly(Side.CLIENT)
	public int getColorFromItemStack(ItemStack itemStack, int damage) {
		return GtiBlocks.blackSesame.getRenderColor(itemStack.getItemDamage());
	}
		...
		if (movingobjectposition == null) {
			...
			if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
				...
				if (world.getBlock(i, j, k).getMaterial() == Material.clay
					...
					world.setBlock(i, j + 1, k, GtiBlocks.blackSesame);
